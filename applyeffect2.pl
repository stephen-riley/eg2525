...
my $effect_function;
for( $effect ) {
    $effect_function = \&sketch when /sketch/;
    $effect_function = \&paper when /paper/;
    $effect_function = \&rough when /rough/;
}
...
while( my $l = shift @lines ) {
    ...
    if( $l =~ /^<g/ ) {
        ...
        $before_group = undef;
        $effect_function->(\@output, \@outside_paths, $l);
        next;
    }
...
sub sketch( $output, $outside_paths, $l ) {
    my( $first ) = $l =~ /(.*?)>/;
...
sub rough( $output, $outside_paths, $l ) {
    my( $first ) = $l =~ /(.*?)>/;
    push @output, "$first $g_line>";
    push @output, qq{<rect style="opacity:0.46;fill:$color;stroke-width:0" id="rect5" width="$w" height="$h" x="$x" y="$y" />};
    push @output, qq{<g id="g99" transform="matrix(0.7,0,0,0.7,29.999997,29.999997)">};
...
sub paper( $output, $outside_paths, $l ) {
    my( $first ) = $l =~ /(.*?)>/;
    push @$output, "$first $g_line>";
    push @$output, qq{<rect style="opacity:0.46;fill:$color;stroke-width:0" id="rect5" width="$w" height="$h" x="$x" y="$y" />};
    push @$output, qq{<g id="g99" transform="matrix(0.7,0,0,0.7,29.999997,29.999997)">};
